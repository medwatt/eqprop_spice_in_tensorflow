...
def load_dataset(dataset_size=1000, x_scale=100.0, y_scale=1.0, normalize=True, reshape=False, stack=False):
...
if __name__ == "__main__":
    ...
    (X_train, Y_train),( X_test, Y_test) = load_dataset(dataset_size=5000, stack=True)
    dataset = create_dataset(X_train, Y_train, batch_size=batch_size)

    model = [
        InputVoltageLayer(shape=(batch_size, 28*28*2), dtype=TF_DTYPE),
        DenseLayer(50, g_range=[1e-7, 1e-5], trainable=True, diodes_enabled=True, diode_params=diode_params, dtype=TF_DTYPE),
        DenseLayer(20, g_range=[1e-7, 1e-5], trainable=True, diodes_enabled=True, diode_params=diode_params2, dtype=TF_DTYPE),
    ...
    ep.train(dataset, epochs=100, free_iter=100, nudge_iter=50, tolerance=1e-6)
