...
if __name__ == "__main__":
    ...
    (X_train, Y_train), (X_test, Y_test) = load_dataset(dataset_size=1000, reshape=True, stack=True)
    x_dummy = tf.expand_dims(X_train[100], axis=0, name=None)
    # x_dummy = X_train[[0, 100]]
    # x_dummy = X_train[[100]]
    # print(x_dummy.shape)
    # exit()

    model = [
        InputVoltageLayer(shape=(1, 28, 28, 2), dtype=TF_DTYPE),
        Conv2DLayer(2, kernel_size=(10, 10), strides=1, padding="valid", g_range=[1e-7, 1e-5], diodes_enabled=False, diode_params=diode_params, dtype=TF_DTYPE),
        # Conv2DLayer(2, kernel_size=(10, 10), strides=1, padding="valid", g_range=[1e-7, 1e-5], diodes_enabled=False, diode_params=diode_params2, dtype=TF_DTYPE),
        DenseLayer(10, g_range=[1e-7, 1e-5], dtype=TF_DTYPE),
        CurrentLayer(current_range=[1e-7, 1e-5], dtype=TF_DTYPE),
    ]
    solver = DCSolver(model)
    voltages = solver.solve_dc(x_dummy, max_iteration=1000, tolerance=1e-6)
